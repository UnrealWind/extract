angular.module('infi-basic')
    .controller('TopicsInputTaskController', ['$rootScope','$scope','DataService','$routeParams','$http','SYS',function ($rootScope,$scope,DataService,$routeParams,$http,SYS){
        $scope.quantityChange = function(data,number){
            for( var i = 0 ; i < $scope.experiment.length ; i++){
                if($scope.experiment[i].name == data.name){
                    $scope.experiment[i].distributionTable.forEach(function(n,iy){
                        var avg = parseInt(number/$scope.experiment[i].distributionTable.length);
                        var remainder = number%$scope.experiment[i].distributionTable.length;
                        iy<remainder?n.number=avg+1:n.number=avg;
                    }) 
                }
            }
        }
        ...
        $scope.quantityChanges = function(data,number){
            for( var i = 0 ; i < $scope.controls.length ; i++){
                if($scope.controls[i].name == data.name){
                    $scope.controls[i].distributionTable.forEach(function(n,iy){
                        var avg = parseInt(number/$scope.controls[i].distributionTable.length);
                        var remainder = number%$scope.controls[i].distributionTable.length;
                        iy<remainder?n.number=avg+1:n.number=avg;
                    }) 
                }
            }
        ...
        DataService.ajaxInputData($routeParams.id).then(function(data){
            ...
            for(var i = 0 ; i < experimentData.length ; i++){
                ...
                for(var ii = 0 ; ii < experimentData[i].taskGroup.length ; ii++){
                    $scope.experiment[i].distributionTable[ii] = {}
                     ...
                     $scope.experiment[i].distributionTable[ii].loadRecord = experimentData[i].taskGroup[ii].loadRecord
                }
            }
            for(var i = 0 ; i < controlsData.length ; i++){
                ...
                for(var ii = 0 ; ii < controlsData[i].taskGroup.length ; ii++){
                    ...
                    $scope.controls[i].distributionTable[ii].loadRecord = controlsData[i].taskGroup[ii].loadRecord
                }
            }
        });
        ...
        $scope.serUser = function(entity){
            $scope.CRFSelectedName = entity.originName;
            $('#set-CRFsave').modal({backdrop:'static'});
            $scope.aa = entity;
        }
        $scope.CRFSelectedSet = function(){
            //点击使用
            var data = $scope.conform.substring(0,3);
            if($scope.listDatas.page && $scope.step == 1) {
                for (var i = 0; i < $scope.listDatas.page.content.length; i++) {
                    $scope.listDatas.page.content[i].backgroundColor = false;
                }
            }
            if($scope.sysListDatas.page&& $scope.step == 2){
            ...
            //console.log(entity);
            $scope.aa.backgroundColor = true;
            if( data == '实验组'){
                for(var i=0;i < $scope.experiment.length;i++){
                    //console.log($scope.experiment[i],66666666666);
                    if($scope.conform == $scope.experiment[i].name){
                        $scope.experiment[i].CRFValue = $scope.aa.originName;
                        $scope.experiment[i].CRFData = $scope.aa;
                    }
                }
            }else if( data == '对照组'){
                for(var i=0;i < $scope.controls.length;i++){
                    if($scope.conform == $scope.controls[i].name){
                        $scope.controls[i].CRFValue = $scope.aa.originName;
                        $scope.controls[i].CRFData = $scope.aa;
                    }
                }
            }
            $('#set-CRFsave').modal('hide');
            $('#set-CrfModal').modal('hide');
        }
        ...
        $scope.setDist = function(data){
            ...
            //当 $scope.HosDep 数组有内容且第一组数据的value为0的话，一定是初始状态的，但凡非初始状态一定都符合这个要求
            function getAvg (){
                if($scope.HosDep.length>0 && $scope.HosDep[0].value === 0){
                    var avg = parseInt($scope.numberPeople/$scope.HosDep.length);
                    var remainder = $scope.numberPeople%$scope.HosDep.length;
                    $scope.HosDep.forEach(function(n,i){
                        i<remainder?n.value=avg+1:n.value=avg;
                    });
                }    
            }
        }
            ...
            if(data == '实验组'){
                for(var ii = 0 ; ii < $scope.experiment.length ; ii++){
                    if($scope.fenPei == $scope.experiment[ii].name){
                        ...
                        !$scope.experiment[ii].distributionTable ? $scope.experiment[ii].distributionTable= []:undefined;
                        for(var i = 0 ; i < $scope.HosDep.length ; i++){
                            !$scope.experiment[ii].distributionTable[i]? $scope.experiment[ii].distributionTable[i]= {}:undefined;
                ...
                for(var ii = 0 ; ii < $scope.controls.length ; ii++){
                    if($scope.fenPei == $scope.controls[ii].name){
                        ...
                        !$scope.controls[ii].distributionTable ? $scope.controls[ii].distributionTable= []:undefined;
                        for(var i = 0 ; i < $scope.HosDep.length ; i++){
                            //$scope.controls[ii].distributionTable[i] = {};
                            !$scope.controls[ii].distributionTable[i]? $scope.controls[ii].distributionTable[i]= {}:undefined;
            ...
            for(var i = 0 ; i < $scope.experiment.length ; i++){
                ...
                for(var ii = 0 ; ii < $scope.experiment[i].distributionTable.length; ii++ ){
                    $scope.subjectGroupsData.taskGroup[ii] = {
                        ...
                        "loadRecord": $scope.experiment[i].distributionTable[ii].loadRecord           //任务分配数量
                    }
                }
            ...
            for(var i = 0 ; i < $scope.controls.length ; i++){
                ...
                for(var ii = 0 ; ii < $scope.controls[i].distributionTable.length; ii++ ){
                    $scope.subjectGroupsData.taskGroup[ii] = {
                        ...
                        "loadRecord": $scope.controls[i].distributionTable[ii].loadRecord           //任务分配数量
                    }
                }
        ...
        $scope.promptMainContentSave = function(){
            if( $scope.promptMainContent == '病例数量不为数字' ) {
                ...
                $scope.btnDels($scope.taskGroupName,$scope.taskGroupNum);
            }
        }
    }]);
