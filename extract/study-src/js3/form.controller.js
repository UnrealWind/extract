angular.module('infi-basic').controller('FormController', ['$scope','formNaviServices','formServices','Upload','FileService','SYS','$routeParams','Session',function($scope,formNaviServices,formServices,Upload,FileService,SYS,$routeParams,Session){
    ...
    $scope.urlBak = '?eu='+userOpt.encodeLoginName+'&ep='+userOpt.password;
	...
	formNaviServices.ajaxNaviData($scope.routeParams).then(function(msg){
		$scope.navigates = msg;
	...
	$scope.changeModule = function(num){
		...
		for(var index in $scope.moduleNameArr){
			if($scope.moduleNameArr[index] === $scope.currModuleName){
				...
				$scope.updateModuleForm($scope.currModuleName);
				$scope.navigates.forEach(function(n,i){
					n.children.forEach(function(ny,iy){
