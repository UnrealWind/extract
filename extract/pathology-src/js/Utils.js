angular.module('infi-basic').service('Utils',[function () {
   ...
   * @param {*} node 页面元素节点
   */
  this.savePageAsImg = function(node) {
    $(node).find('.sec-item-span-label').css('width', '190px')
    ...
    .then(function (dataUrl) {
        ...
        link.href = dataUrl;
        link.click();
        $(node).find('.sec-item-span-label').attr('style', '')
    });
  }
   ...
   * @param {*} opts 配置项
   */
  this.printPage = function(node, opts) {
      $(node).print(opts || {})
  }
