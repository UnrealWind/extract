...
function node_onMouseOver(d,type) {
    if (type=="CAND") {// 点  cands里的数据
        ...
        var titleTip = d.FILE_NUM || pacsById[office + "_" + d.CMTE_ID].CMTE_NM;  //得到线连接的弧的科室
        // 设置图上线的显示title
        ...
        header2.text(formatCurrency(Number(d.TRANSACTION_AMT)) +'人次 占’'+ titleTip+'‘多科就诊总人数的'
...
function highlightLink(g,on) {
    ...
    //注:150:550 不同的d3版本就此内容的渲染不同,需要根据需要调整比例
