angular.module('infi-basic').controller('RecordController', ['$scope', 'SYS', 'RecordService', 'EMR', '$timeout', '$sce', function ($scope, SYS, RecordService, EMR, $timeout,$sce) {
   ...
   // 列表页（首页）对应controller
    ...
    $scope.getCaseList = function (page, size, keywords) {
        ...
        RecordService.getPage(page, size, keywords).then(function (msg) {
            ...
            //添加状态按钮
            if(msg.success == true){
                RecordService.filterBtn($scope.listData);
            }
        });
    };
