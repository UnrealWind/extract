angular.module('infi-basic').service('RecordService', ['$http', 'SYS', '$filter', function ($http, SYS, $filter) {
    ...
    function convertObject2BooleanValue(type, entity) {
        ...
        if (angular.isArray(entity)) {
            ...
            for (idx in entity) {
                if (entity[idx] != undefined && entity[idx] != "" && isCheckbox(type, idx)) {
    ...
    function convertBooleanValue2Object(entity) {
        ...
        if (angular.isArray(entity)) {
            ...
            for (idx in entity) {//此处idx不是索引,是entity中的一项
                if (( entity[idx] && angular.isObject(entity[idx])) || ( entity[idx + 'OTHER'] && angular.isObject(entity[idx + 'OTHER']))) {
                    ...
                    if (!angular.isString(values) || values=="") {
                        //zyz_debug 解决bug
                        //场景:  多选值保存(多选有添加其他的情况)当默认存在的选项全部没有选,只有添加的选项,values为""字符串格式,
                        // 会被阻挡在if条件之外,因为后台返回时values是字符串,所以在if语句中添加为""的情况,并赋给它一个空对象
                        if(values == ""){
                            values = {};
                        }
     ...
     * @param listData
     */
    this.filterBtn = function(listData){
        angular.forEach(listData.result,function(list){
            if(list.status == '未创建'){
                list.btnLabel = "创建产科档案";
                list.btnUrl = "#/record/input/"+list.xl_patient_id + "/"+list.pati_id+"/first";
            }else if(list.status == '产后42天'){
                list.btnLabel = "查看产科档案";
                list.btnUrl = "#/overview/obstetrics/"+list.xl_patient_id + "/"+list.xl_medical_id+"/"+list.pati_id;
                list.btnLabel2 = "编辑产后42天";
                list.btnUrl2 = "#/record/input/"+list.xl_patient_id + "/" + list.xl_medical_id + "/"+list.pati_id+"/third";
            }else if(list.status == '关闭'){
                list.btnLabel = "查看产科档案";
                list.btnUrl = "#/overview/obstetrics/"+list.xl_patient_id + "/"+list.xl_medical_id+"/"+list.pati_id;
                list.btnLabel2 = "创建产科档案";
                list.btnUrl2 = "#/record/input/"+list.xl_patient_id + "/"+list.pati_id+"/first";
            }else{
                list.btnLabel = "查看产科档案";
                list.btnUrl = "#/overview/obstetrics/"+list.xl_patient_id + "/"+list.xl_medical_id+"/"+list.pati_id;
                var data = $filter('date')(new Date(),'yyyy-MM-dd');
                if(data == list.record_date){
                    list.btnLabel2 = "编辑孕检记录";
                }else{
                    list.btnLabel2 = "创建孕检记录";
                }
                list.btnUrl2 = "#/record/input/second/"+list.xl_patient_id + "/"+list.xl_medical_id+"/"+list.pati_id;
            }
        })
    };
}]);
