...
function buildChords() {
    ...
    var totalPacAmount=0; //arcs的Amount总和

    // gqm_debug 并不知道这里有什么用处
    // Compute a unique index for each package name
    pacs.forEach(function(d) {
        d = d.CMTE_ID;
        if (!(d in indexByName)) {
            nameByIndex[n] = d;  //nameByIndex[0/1/2..] arcs的id数组 = indexByName
            indexByName[d] = n++;
        }
    });
    ...
    pacs.forEach(function(d) {
        ...
        row[indexByName[d.CMTE_ID]]= Number(d.Amount);  //row[0]=[201,0,0,0..]/row[1]=[0,234,0,0,0...]/row[3]=[0,0,123.0.0..]....
        totalPacAmount+=Number(d.Amount);   //arcs的Amount总和
    });

    //  console.log("totalPacAmount=" + totalPacAmount)
    // gqm_debug 不太理解chord.matrix的作用
    chord.matrix(matrix);  //matrix为row的集合
    ...
    chords.forEach(function (d) {
        ...
        o.source= d.source;
        o.relatedLinks=[];
        chordsById[d.label]= o;  //chordsById[CMTE_ID] 弧上的一些设置，包括位置，x、y等
        i++;
    });
