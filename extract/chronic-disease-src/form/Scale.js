		...
		link:function(scope,element,attrs){
			...
			scope.calcScale = function() {
				...
				utils.recur(scope.modelData.questionList, function(ele) {
					if(ele.choicedOptionList && ele.choicedOptionList.length > 0) checkedRad = checkedRad.concat(ele.choicedOptionList)
				})

				scope.calcRst.changeState('calc')																		// 更改状态为 计算中(calclating)

				// 发送选中项， 获得计算结果
				$http.post(`${SYS.url}research/evoluation/form/${scope.input.scaleUrl}/result`, checkedRad).then(function (msg) {
