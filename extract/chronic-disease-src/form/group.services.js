angular.module("infi-basic").service('groupServices',['$http','SYS','Session',function($http,SYS,Session){
    ...
    this.getScaleData = function(input){
		...
		// 慢病自个量表模板数据获取方法
		this.chronGetScaleData = function (input, resultId) {
			var id = input.scaleUrl																												// 量表的 id
			var url = `${SYS.url}research/evoluation/form/${id}/content?resultId=${resultId}`									// 量表模板数据获取接口
