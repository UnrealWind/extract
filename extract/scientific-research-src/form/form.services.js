...
angular.module('infi-basic').service('formServices',['$http','SYS','Session','$routeParams',
    ...
    this.saveModule = function($scope,url){
		!url?url = SYS.url+'subject/'+$routeParams.subjectId+'/group/'+$routeParams.groupId+'/'+$scope.currentModule.templateId+'/'+$scope.sessionInterview.id+'/'+$scope.currentModule.name:'';
    ...
    this.saveAllModule = function (interview) {
        return $http({
            ...
            //history.go(-1);
            window.location.href= '#/record-list/'+$routeParams.subjectId+'/'+$routeParams.groupId;
            return msg;
        });
    }
