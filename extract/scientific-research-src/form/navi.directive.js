angular.module('infi-basic').directive('affix',['$http','naviServices','$routeParams','SYS','$timeout',function($http,naviServices,$routeParams,SYS,$timeout){
	return {
		restrict: 'ECMA',
        template: '<div><div><div id="affix" class="affix-nav" role="complementary">'
					...
					+'</div>'
					+'<div class="addCRF">'
						+'<span ng-click="addCRFTemplate(6)">+ 添加调查问卷</span>'
        				+'<span ng-click="addCRFTemplate(7)">+ 添加评分表</span>'
					+'</div>' +
