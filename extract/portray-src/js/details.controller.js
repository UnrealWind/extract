angular.module('infi-basic').controller('DetailsController', ['$scope', 'SYS', 'DetailsService', '$routeParams', '$location', function ($scope, SYS, DetailsService, $routeParams, $location) {
    ...
    $scope.params.signature = $location.search().signature;
    $scope.params.caseId = $location.search().caseId;
    ...
    function init() {
        ...
        // DetailsService.getInfo($scope.params).then(function(msg){
            ...
            //获取图需要的数据
            DetailsService.getOrganJson(json).then(function(msg){
                $scope.organJson = msg.data;
                DetailsService.getPositionJson().then(function(msg){
