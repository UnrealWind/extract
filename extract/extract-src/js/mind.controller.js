angular.module("infi-basic").controller('MindController',['$scope','SYS','Session','$routeParams',function ($scope,SYS,Session,$routeParams) {
    ...
    function drawChart(data) {
        ...
        //调取数据进行绘图
        d3.json(SYS.url + url +'&eu='+users.eu+'&ep='+users.ep, function ( error,root) {
                    ...
                    .on("click", function (d) {  //给添加的节点绑定click事件
                        if(d.children||d._children){  //有子集的点击展开或者收起子集
                            ...
                            //特殊属性值属性查看导图再次进入属性值或者特殊属性值属性列表,要传入此属性的id
                                url = $scope.child.id ? d.id+'/': "";
                            window.open(path+'/extract-src/#/mind-detail/'+url+d.key+'/'+d.tableName);
                        }
                    })
        ...
        function findChildren(entity,index) {
            ...
            if(entity.resourceTableName||entity.specialResourceTableName){
                ...
                entity.resourceTableName?entity.children.unshift(
                    {id:entity.value,name:"属性值",tableName:entity.resourceTableName,key:"resource"}):undefined;
            }
            setOnlyName(entity);
            if(entity.children){
                xIndex = ++index;
