angular.module("infi-basic").controller('MindDetailController',['$scope','SYS','Session','$routeParams','$http',function ($scope,SYS,Session,$routeParams,$http) {
    $scope.SYS = SYS;
    ...
    $scope.goToMind = function (page) {
        if(page.tableName){
            ...
            window.open(path+'/extract-src/#/mind/'+page.id+'/'+page.name+'/'+page.tableName);
        }
    }
